...
const HostNics3 = ({ hostId }) => {
  ...
  // 변경 값
  const [movedItems, setMovedItems] = useState({ nic: [], networkAttachment: [], network: []});
  ...
  // 네트워크 인터페이스 목록
  const transNicData =  useMemo(() => { return [...baseItems.nic]
  ...
  // 할당된 네트워크
  const transNAData = useMemo(() => { return networkAttachments
  // const transNAData = useMemo(() => { return [...baseItems.networkAttachment]
    .map((e) => {
  ...
  const transDetachNetworkData = useMemo(() => { return networks
  // const transDetachNetworkData = useMemo(() => { return [...baseItems.network]
