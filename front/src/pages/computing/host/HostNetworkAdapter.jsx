...
const HostNetworkAdapter = ({
  ...
  const transformedData = hostNics
    ...
    .map((e) => {
      const isBonding = e?.bondingVo?.slaveVos?.length > 0;
      const bondSlaves = isBonding
        ? e.bondingVo.slaveVos.map((s) => s.name).join(", ") 
        : null;

      return {
        ...e,
        id: e?.id,
        name: isBonding ? `${e.name} [ ${bondSlaves} ]` : e.name, // name 변경 처리
        bondingVo: {
          // ...e?.bondingVo,
          activeSlave: {
