...
class VWAuthController {
	...
	@ApiImplicitParams(
		ApiImplicitParam(name="username", value="VMWare 사용자 ID", dataTypeClass=String::class, required=true, paramType="path"),
		ApiImplicitParam(name="passwordPrompt", value="VMWare 사용자 비밀번호", dataTypeClass=PasswordPrompt::class, required=true, paramType="body"),
	)
	@ApiResponses(
	...
	): ResponseEntity<VMWareSessionId> {
		log.info("vw/auth/{}, password: {}", username, passwordPrompt.password)
		if (username.isEmpty())
			throw ErrorPattern.OVIRTUSER_ID_NOT_FOUND.toException()
		if (passwordPrompt.password.isEmpty())
			throw ErrorPattern.OVIRTUSER_REQUIRED_VALUE_EMPTY.toException()

		val res: VMWareSessionId = iVWAuth.createSession(username, passwordPrompt.password)
