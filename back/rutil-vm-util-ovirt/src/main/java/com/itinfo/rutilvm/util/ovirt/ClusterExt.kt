...
fun Connection.findCluster(
	...
	throw if (it is Error) it.toItCloudException(Term.CLUSTER, "상세조회", clusterId) else it
}
...
fun Connection.addCluster(cluster: Cluster): Result<Cluster?> = runCatching {
	...
	throw if (it is Error) it.toItCloudException(Term.CLUSTER, "생성") else it
}
...
fun Connection.updateCluster(cluster: Cluster): Result<Cluster?> = runCatching {
	...
	throw if (it is Error) it.toItCloudException(Term.CLUSTER, "편집", cluster.id()) else it
}
...
fun Connection.removeCluster(clusterId: String): Result<Boolean> = runCatching {
	...
	throw if (it is Error) it.toItCloudException(Term.CLUSTER, "삭제", clusterId) else it
}
...
fun Connection.findAllHostsFromCluster(clusterId: String, follow: String = ""): Result<List<Host>> = runCatching {
	...
	throw if (it is Error) it.toItCloudExceptionWithin(Term.CLUSTER, Term.HOST,"목록조회", clusterId) else it
}
...
fun Connection.findAllVmsFromCluster(clusterId: String, searchQuery: String = "",follow: String = ""): Result<List<Vm>> = runCatching {
	...
	throw if (it is Error) it.toItCloudExceptionWithin(Term.CLUSTER, Term.VM,"목록조회", clusterId) else it
}
...
fun Connection.findAllNetworksFromCluster(clusterId: String, follow: String = ""): Result<List<Network>> = runCatching {
	...
	throw if (it is Error) it.toItCloudExceptionWithin(Term.CLUSTER, Term.NETWORK,"목록조회", clusterId) else it
}
...
fun Connection.findNetworkFromCluster(clusterId: String, networkId: String): Result<Network?> = runCatching {
	...
	throw if (it is Error) it.toItCloudExceptionWithin(Term.CLUSTER, Term.NETWORK,"상세조회", clusterId, networkId) else it
}
...
fun Connection.addNetworkFromCluster(clusterId: String, network: Network): Result<Network?> = runCatching {
	...
	throw if (it is Error) it.toItCloudExceptionWithin(Term.CLUSTER, Term.NETWORK,"생성", clusterId) else it
}
